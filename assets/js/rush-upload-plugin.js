...
function handleFiles(files) {
  ...
  for (let i = 0; i < Math.min(files.length, MAX_IMAGES - imageCount); i++) {
    ...
    if (!validationResult.isValid) {
      feedback.invalidFiles.push({ name: file.name, reason: validationResult.message });
      continue;
    }

    if (addedFiles.includes(file.name)) {
      feedback.duplicateFiles.push(file.name);
      continue;
    }
  ...
  if (feedback.duplicateFiles.length > 0) {
    console.log(`Arquivo duplicado detectado: ${feedback.duplicateFiles.join(', ')}.`);
  }

  if (feedback.invalidFiles.length > 0) {
    feedback.invalidFiles.forEach(file => {
      console.log(`Imagem inválida: "${file.name}." Motivo: ${file.reason}`);
    });
  }

  updateUIWithFeedback(feedback)
}
function updateUIWithFeedback(feedback) {
  const feedbackContainer = document.getElementById('feedbackContainer');
  feedbackContainer.innerHTML = ''; // Clear previous feedback

  if (feedback.invalidFiles.length > 0) {
    const invalidFilesMsg = feedback.invalidFiles.map(f => `Imagem inválida: "${f.name}." Motivo: ${f.reason}`).join('<br>');
    feedbackContainer.innerHTML += `<p>${invalidFilesMsg}</p>`;
  }

  if (feedback.duplicateFiles.length > 0) {
    const duplicateFilesMsg = `Arquivo duplicado detectado: ${feedback.duplicateFiles.join(', ')}.`;
    feedbackContainer.innerHTML += `<p>${duplicateFilesMsg}</p>`;
  }
}

// Call updateUIWithFeedback(feedback) at the end of handleFiles function
...
function isValidImage(file) {
  ...
  if (!allowedExtensions.includes(`.${extension}`)) {
    return { isValid: false, message: "Extensão não aceita" };
  }

  if (!allowedMimeTypes.includes(file.type)) {
    return { isValid: false, message: "Tipo MIME não aceito" };
  }

  if (file.size > maxFileSize) {
    return { isValid: false, message: `Arquivo excedeu o limite de ${MAX_FILE_SIZE}MB` };
  }
...
function updateAddMoreBox() {
  ...
  addMoreBox.innerText = `Restam \n${remains.toString().padStart(2, '0')}\n ${remains >= 2 ? 'imagens' : 'imagem'}`;
